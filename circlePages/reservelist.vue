	...
	<tn-nav-bar fixed alpha :isBack="true" >
		...
		<view class='nav-list tn-margin-bottom tn-margin-top'  :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			...
			<block v-for="(item, index) in content" :key="index" >
				<view class="nav-list-item tn-shadow-blur tn-cool-bg-image tn-main-gradient-red--light"
					:class="['tn-main-gradient-' + item.color + '--light']" @click="navigateToFn({ url: '/circlePages/blogger_other?id='+item.user_id, checkLogin: false })">
					<view class="nav-link">
						<view class='title tn-text-bold' style="color: #080808;">{{ item.nickname }}</view>
						<view class='juli tn-color-grey tn-text-sm clamp-text-2' style="color: #080808;">{{ item.create_time }}</view>
						<view class='join tn-color-grey tn-text-sm clamp-text-2' v-if="item.user_id==item.myid">{{ item.mobile }}</view>
					</view>
					<view class="icon tn-shadow-blur tn-bg-bule"  style="opacity: 0.5;">
