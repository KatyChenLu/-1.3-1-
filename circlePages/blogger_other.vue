<template>
	<view class="template-mine tn-safe-area-inset-bottom">
		...
		<!-- #ifdef APP-PLUS||H5 -->
			<view slot="right" class="tn-flex tn-flex-col-right tn-flex-row-right">
				<view class="custom-nav__right" style="margin-right: 10px;">
					<text class=" tn-icon-share-square tn-color-white" @click="appshare()" style="font-size: 20px;"></text>
			</view>			
			</view>
		<!-- #endif -->
		...
		<mescroll-body ref="mescrollRef" @down="downCallback" @up="upCallback">
			...
			<view class="about__wrap">
				<view style="width: 150px;height: 150px; 
    ...
    z-index: 120;
    top:50px;
	right: 10px;" v-if="body.chongwu">
					<image v-model="showchongwu" :src="body.chongwu" style="height: 100px;" mode="heightFix"></image> 
				</view>
					...
					<view class="tn-flex tn-flex-row-between tn-flex-col-center tn-margin-bottom">
						<view class="justify-content-item" style="width: 100%;">
							<view class="tn-flex tn-flex-col-center tn-flex-row-left">
								<view class="logo-pic " @click="datu(body.avatar)">
								...
								<view class="" style="width:540rpx">
									...
									<view class="tn-padding-right tn-padding-left-sm tn-text-xl tn-text-bold">
										...
										<text class="" :class="{'tn-color-red': body.vip===1,'tn-color-white': body.vip===0}">{{body.nickname}}</text>
										...
										<text class="tn-icon-sex  tn-color-white" v-if="body.sex==0"></text>
										<text class="dk-lv tn-main-gradient-indigo--light" v-if="body.vip==0&&configInfo.is_lv">LV {{body.lv}}</text>
										<text class="dk-vip tn-main-gradient-orangeyellow" v-if="body.vip==1"><text class="tn-icon-vip tn-color-black"
									...
									<view class="tn-padding-right tn-padding-top-xs tn-padding-left tn-text-ellipsis">									
										<text v-if="body.online==1" class="tn-tag-class tn-tag tn-color-white tn-cool-bg-color-6 " style="padding: 0 5px;font-size: 12px;margin-right:3px;">
										在线</text>
									
										<text v-if="body.is_dav==1"
											class=" tn-icon-success-circle-fill tn-color-red"
									...
									<view
										class="tn-padding-right tn-padding-top-xs tn-padding-bottom-sm tn-padding-left-sm" @click="navigateToFn({ url: '/circlePages/medal?id='+id, checkLogin: false })">
										<block v-for="(item,indexdd) in body.medal_list" :key="indexdd">
											<tn-tag backgroundColor="tn-cool-bg-color-4" margin="10rpx 10rpx">
												{{item.title}}
											</tn-tag>
				...
				<view class="tn-margin-top">
					<view class="tn-flex tn-flex-row-center">
						...
						<tn-tabs :list="scrollList" :itemWidth="220" :activeItemStyle="activeItemStyle" :current="currenttiezi"
					...
					<block v-if="currenttiezi==0">
						...
						<view
							class="tn-tag-content tn-margin-left-sm tn-margin-top tn-margin-right-sm  tn-text-justify ">
						 <text v-if="body.shengri">
							{{body.shengri.age}}，{{body.shengri.animals}}，{{body.shengri.constellation}}
						</text>
						<text v-if="body.tizhong">
							，{{body.tizhong}}Kg
						</text>
						<text v-if="body.shengao">
							，{{body.shengao}}cm
						</text>
						<text v-if="body.zhiye">
							，{{body.zhiye}}
						</text>
						</view>
						<view v-if="body.wxhao"
							class="tn-tag-content tn-margin-left-sm tn-margin-top tn-margin-right-sm  tn-text-justify tn-padding-bottom">
						微信号：<text v-if="body.vipziji==0&&body.vipkanweixn==0">{{body.wxhao}} <text style="padding:5px 10px;
			font-size: 12px;
			height: 18px;
			margin: 5px 0 5px 8px;
			width: auto;
			" class="tn-bg-red  tn-btn-class tn-color-white tn-round"
						@click="fuzhi(body.wxhao)">
						复制
					</text>
					</text>
					<text v-else>vip可查看
					...
					width: auto;
					" class="tn-bg-red  tn-btn-class tn-color-white tn-round"
								@click="navigateToFn({ url: '/minePages/vip', checkLogin: true })">
								去开通
							</text>
					</text>
						</view>
						...
						<view
							class="tn-tag-content tn-margin-left-sm tn-margin-right-sm  tn-text-justify tn-padding-bottom"
							v-if="body.tags.length>1">


							<view v-for="(item,indexsdsf) in body.tags" :key="indexsdsf"	class="tn-tag-content__item tn-margin-right tn-round tn-text-sm tn-text-bold tn-bg-green--light tn-color-black">
								{{item}}
							</view>
						...
						<view class="tn-flex tn-flex-row-between " v-if="body.voc">
							<view class="justify-content-item tn-margin-left-sm tn-margin-right-sm tn-text-bold">
								语音签名
							</view>
						</view>
						<view class="tn-tag-content tn-margin-left-sm tn-margin-right-sm tn-padding-top-sm"
							...
							<tn-button backgroundColor="#00FFC6" padding="40rpx 0" width="60%" shadow fontBold
								...
								<view class="voice-line-wrap"
									...
									<text class="tn-color-black" style="margin-left: 15px;">{{body.vocsen}} ''</text>
								</view>
						...
						<view class="tn-flex tn-flex-row-between tn-padding-top-sm">
							<view class="justify-content-item tn-margin-left-sm tn-margin-right-sm tn-text-bold">
								收到的礼物({{body.giftCount}})
							</view>
							<view class="justify-content-item tn-margin-right-sm tn-text-lg tn-color-grey"
								@click="navigateToFn({ url: '/circlePages/gift_user?id='+id, checkLogin: false })">
								<text class="tn-icon-right"></text>
							</view>
						</view>
						<view class="tn-tag-content tn-margin-left-sm tn-margin-right-sm tn-padding-top-sm">
							<tn-scroll-list v-if="body.giftlog.length>0">
								<view class="tn-flex ">
									<block v-for="(item, indexgift) in body.giftlog" :key="indexgift">
										<view style="width: 100px; height: 140px;background-color: #f8f8f8;"
											class=" tn-flex-1 tn-padding-sm tn-margin-xs tn-radius "
											>
											<view class=" tn-flex-direction-column " style="text-align: center;
    align-items: center;">
												<image class="" :src="item.image" style="width: 60px; height: 60px;">
												</image>
												<view class="tn-color-black tn-text-center">
													<text class="tn-text-ellipsis">{{item.title}}</text>
												</view>
												<view class="tn-color-black tn-text-center tn-padding-top-sm">
													<text
														class="tn-text-ellipsis tn-color-gray">x{{item.gift_num}}</text>
												</view>
											</view>
										</view>
									</block>
							...
							<tn-empty icon="/static/images/no.png" text="暂无礼物,快去互动吧!" v-else>

							</tn-empty>

						</view>
					</block>
					<block v-if="currenttiezi==2">
						<view class="tn-tag-content   tn-padding-top-sm">
							<tn-empty icon="/static/images/no.png" text="空空如也~~~" v-if="content.length==0" style="height: 280px;">
							</tn-empty>
					...
					<block v-if="currenttiezi==1">
					
	<tn-empty icon="/static/images/no.png" text="空空如也~~~" v-if="goodslist.length==0" style="height: 280px;">
							</tn-empty>
							
	<view v-else class="tn-flex tn-flex-col-top tn-bg-gray--light  tn-shadow tn-padding tn-margin" v-for="(item,indexdd) in goodslist" :key="indexdd"  >
	  <view class="" >
	    <view class="logo-pic ">
	  ...
	  <view class="tn-padding-left-sm tn-text-ellipsis" style="width: 100%;" >
	    ...
	    <view class="  ">
	      <text class=" ">{{item.dec}}</text>
	    </view>	
		<view class="  ">
		  <text class="tn-color-red ">{{configInfo.site_moneyname}}{{item.price}}/{{item.danwei}}</text>
		</view>	
				  
	  </view>
	  <view class="justify-content-item tn-round tn-text-xs  tn-color-white" style="padding: 10rpx 20rpx;" >
	  	  
	  				<tn-tag backgroundColor="tn-cool-bg-color-7" fontColor="tn-color-white" shape="circle"
	  					width="auto" size="lg" style="margin-left: 15px;" @click="navigateToFn({ url: '/circlePages/blogger_other_user?id='+item.user_id+'&game_id='+item.id, checkLogin: false })">
	  				下单</tn-tag>
	  </view>
		...
		<view class="input-box">
			...
			<!-- 	//opengift() -->
			<view @click="opengift()"
				class="blogger__author__btn justify-content-item tn-flex-col-center tn-flex-row-center tn-margin-left-sm"
				style="margin-top:15px;">
				<text class="tn-bg-orangered--light tn-icon-gift  tn-round  tn-text-bold tn-color-black"
					style="padding: 15rpx 28rpx;font-size: 14px;">
					礼物</text>
			...
			<view class="send " @tap="sendmsg">
				<view class="btn tn-icon-empty-comment" style="font-size: 20px;">
					<text style="margin-left: 5px;font-size:16px;">私 聊 </text>
				</view>
			</view>
		</view>
		...
		<tn-popup v-model="shouhuxiu" width="80%" height="50%" mode="bottom" :borderRadius="30" :closeBtn="true"
			:maskCloseable="true" @close="closedPopup">

			<view class="tn-margin-top-sm">
				<view style="align-items: center;border-radius: 25px;height: 100px;
    justify-content: center;b" class="tn-flex tn-flex-row-between  tn-padding-xs tn-margin-xs">
					<view class="tn-flex tn-flex-direction-column tn-flex-row-center tn-flex-col-center">

						<tn-avatar class="" shape="circle" :src="body.avatar" size="xl">
						</tn-avatar>
						<view class="tn-color-black tn-text-center tn-text-justify tn-padding-top-sm">
							<text class="tn-color-black ">守护 {{body.nickname}}</text>
						</view>
					</view>
				</view>
			</view>
			<view class="tn-flex tn-flex-row-around tn-margin-top-sm money-area ">
				<view
					...
					v-for="(item, indexsshhud)  in body.keep_price" :key="item"
			...
			<view class="tn-text-center" @click="shouhuosend">
				<tn-button :shadow="true" width="60%" backgroundColor="#ffbc49" height="80rpx" margin="10rpx 0">立即开通
				</tn-button>
			</view>
	...
	<tn-modal width="90%" v-model="payshow" backgroundColor="#E4E9EC" padding="30rpx 20rpx" :radius="25"
				title="下单" :showCloseBtn="true" :maskCloseable="false" :zoom="true" :custom="true">
		
				<view class="article-shadow tn-margin-sm tn-bg-white tn-padding-sm">
					<view class="tn-flex">
