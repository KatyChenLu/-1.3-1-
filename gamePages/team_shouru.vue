<template>
    ...
    <!-- <view class="status-bar"></view> -->
		<view class='my-promotion' :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			<view class="header">
				<view class='name acea-row row-center-wrapper'>
					<view>{{gift_tixian_name}}总收入</view>
				</view>
				<view class='num'>
         {{zong_money}}
				...
				<view class='profit acea-row row-between-wrapper'>
					<view class='item'>
						<view >可提现{{gift_tixian_name}}</view>
						<view class='money'>{{gift_money}}</view>
					</view>
					<view class='item'>
						<view>已提现{{gift_tixian_name}}</view>
						<view class='money'>{{yitixian}}</view>
					</view>
				</view>
			</view>
	
	
			<view @click="shezhishow=true" class='bnt bg-color'>立即提现</view>
			...
			<tn-modal width="80%" v-model="shezhishow" backgroundColor="#ffffff" padding="30rpx 20rpx" :radius="25"
				...
				<view class="custom-modal-content tn-padding-sm ">
					...
					<input placeholder="请输入提现数量" type="number" v-model="ti_money" placeholder-style="color:#343434"
					...
					<!-- #endif -->
					<!-- #ifdef MP||H5 -->
					<block v-if="is_wx_tixian" >
						<view class='tn-margin'>本站开启提现到微信零钱包，后台审核后请到微信查看！</view>
					</block>
					<block v-else>
					<view class="tn-bg-gray--light "
						style=" margin-top:8px; border-radius: 10rpx;padding: 20rpx 30rpx;">
