<template>
	...
	<tn-nav-bar fixed :bottomShadow="false">我的服务订单</tn-nav-bar>	
				<!-- 顶部标签 -->
						...
						<view v-for="(item,index) in content " :key="index" class="order__item  tn-border-solid tn-shadow" >
							...
							<view
								...
								<view class="order__item__operaation__left">
									...
									<text class="order__item__operaation__left--text tn-color-red tn-margin-left" v-if="type==2">收入{{item.shouprice}}</text>
								...
								<view class="order__item__operaation__right__button" v-if="item.status==0&&type==2" @click="queren(item.id)">
								<tn-button shadow shape="round" fontColor="#ffffff" backgroundColor="#00aaff" :fontSize="24" height="auto" padding="10rpx 28rpx" >
										确认接单
								</tn-button>											
								</view>
								
								<view class="order__item__operaation__right__button" v-if="item.status==1&&configInfo.is_pepei_wan" @click="wancheng(item.id)">
								<tn-button shadow shape="round" fontColor="#ffffff" backgroundColor="#00aaff" :fontSize="24" height="auto" padding="10rpx 28rpx" >
										完成订单
								</tn-button>											
								</view>
								
								<view class="order__item__operaation__right__button" v-if="item.is_tui==1&&type==2" @click="tongyi(item.id)">
