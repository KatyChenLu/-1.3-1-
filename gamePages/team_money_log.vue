...
<tn-list-cell class="tn-border-solid tn-shadow tn-margin-top" :hover="true" :unlined="true" :radius="true" :fontSize="30"  v-for="(item, index) in content" :key="index" v-if="content.length>0" >
					  <view class="tn-flex tn-flex-col-center ">					   
					    <view class=" tn-flex-1">陪陪：{{item.nickname}}</view>
					    <view class="tn-margin-left-sm tn-color-red ">{{item.money}}个</view>
					  </view>
					    <view class="tn-flex tn-flex-col-center tn-margin-top">	
					  {{ item.description }}时间：{{ item.create_time }}
					   </view>
					   <view class="tn-flex tn-flex-col-center tn-margin-top">
					   订单号：{{ item.other_id }}
					    </view>
