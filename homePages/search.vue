<template>
      ...
      <view class="tn-flex tn-flex-row-between tn-flex-col-center tn-margin" :style="{marginTop: vuex_custom_bar_height + 'px'}">
        <view class="justify-content-item align-content-item">
          <view class="tn-bg-gray--light tn-flex tn-flex-col-center" style="border-radius: 100rpx;padding: 10rpx 20rpx 10rpx 20rpx;width: 100%;">
            ...
            <input class="justify-content-item" placeholder="输入帖子或ID号或昵称" v-model="searchKey" placeholder-style="color:#AAAAAA" ></input>
          </view>
        </view>
 ...
 <view class="tn-flex tn-flex-row-between tn-margin-sm" >
 	...
 	<view class="" style="width: 100%;">
 		<tn-tabs-swiper class="" ref="tabs" :bold="true" activeColor="#000000" inactiveColor="#8b8b8b":list="scrollList" :fontSize="30" :current="currenttiezi" :isScroll="false" @change="tabsChange"></tn-tabs-swiper>
 	</view>
 </view>
 
 
	<view class="tn-flex tn-flex-direction-column tn-margin-top-sm tn-margin-bottom" v-if="currenttiezi==0">	
		<quan-list :list="content" :index="true"></quan-list>
	</view>
	
	<view class="nav-list tn-margin-bottom tn-margin-top" v-if="currenttiezi==1">	
		
		<block v-for="(item, index) in content" :key="index" >
			<view class="nav-list-item tn-shadow-blur tn-cool-bg-image tn-main-gradient-blue--light"
			 @click="navigateToFn({ url: '/circlePages/blogger_other?id='+item.id, checkLogin: false })">
				<view class="nav-link">
					...
					<view class='joinid tn-color-grey tn-text-sm'>ID：{{ item.id }}</view>
					<view class='join tn-color-grey tn-text-sm clamp-text-2'>{{ item.dec }}</view>
				</view>
				<view class="icon tn-shadow-blur tn-bg-blue"  style="opacity: 0.5;">
...
<script>
    ...
    methods: {
	  ...
	  async upCallback(mescroll) {
		...
		let url
		  if (this.currenttiezi==0){
			   url="/api/index/getGoodslist"
		  }
		  if (this.currenttiezi==1){
		  			   url="/api/index/getuserlist"
		  }
		  if (this.currenttiezi==2){
		  			   url="/api/index/getGoodslist"
		  }
