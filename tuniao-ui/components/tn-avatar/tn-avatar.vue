<template>
  <view
	...
	text-align: center;
	pointer-events: none;" v-if="bg">
	<image  :class="[avatarClassbg]" mode="aspectFill" :src="bg"></image>
	</view>
	
    <view v-if="!showImg" class="tn-avatar__text" >
      <view v-if="text">{{ text }}</view>
      <view v-else :class="[`tn-icon-${icon}`]"></view>
    </view>
	 <view class="" style="position: absolute;bottom:0px" v-if="text_center">
    <view class="dk-shouhu">{{text_center}}</view>
		 </view>
    <!-- 角标 -->
    <tn-badge
...
<script>
  ...
  export default {
    ...
    computed: {
	  ...
	  avatarClassbg() {
	    let clazzbg = ''
	    clazzbg += ` tn-avatargb--${this.shape}`
	    
	    if (this._checkSizeIsInline()) {
	      clazzbg += ` tn-avatargb--${this.size}`
	    }
