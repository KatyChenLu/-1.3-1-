<template>
  <view
    ...
    style="width: 140px;height:30px;border-radius:25px;margin: 5px;"
    @tap="click"
  >
    <view
    ...
    <!-- 左图标 -->
    <view  class="tn-switch__icon tn-switch__icon--left"   
	 :class="[
	   `tn-icon-${leftIcon}`,
	   value ? 'tn-switch__icon--show' : ''
	 ]" 
      :style="[iconStyle]">{{titlezuo}}</view>
    <!-- 右图标 -->
    <view  class="tn-switch__icon tn-switch__icon--right" 
	:class="[
        `tn-icon-${rightIcon}`,
        !value ? 'tn-switch__icon--show' : ''
      ]"    
      :style="[iconStyle]">{{titleyou}}</view>
  </view>
</template>
    ...
    computed: {
      switchStyle() {
        ...
        style.backgroundColor = this.value ? 
          this.activeColor ? this.activeColor : '#AAAAAA' : 
    ...
    methods: {
      click() {
        if (!this.disabled && !this.loading) {
		 ...
		 if (this.value){
			this.title=this.titlezuo 
		 }
		 else{
			this.title=this.titleyou 
		 }
          this.$nextTick(() => {
