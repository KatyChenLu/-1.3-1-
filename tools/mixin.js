	...
	methods: {
			...
			return new Promise(async (res) => {			
				...
				if (result.statusCode == 200) {
					if (result.data.code == 200) {
						...
						that.$store.commit('setConfigInfo', resultObj);									
						res(resultObj)
					} else {
						uni.showToast({
							icon: 'none',
			...
			return new Promise(async (resFn, rejFn) => {
				...
				if (result.statusCode == 200) {
					if (result.data.code == 200) {					
						...
						let userInfoMoney = parseInt(userInfoObj.money) // 余额
						let userInfoCoupon = userInfoObj.coupon // 优惠券数量
						let userInfoMc = userInfoObj.mc // 商品券
						let userInfoIntegral =parseInt(userInfoObj.integral) // 积分
						...
						that.$store.commit('setmoney',userInfoObj.money);
						that.$store.commit('setsore',userInfoObj.integral);
						// 个人中心头部的数据 - 卡片里的东西
		...
		*/
		...
		logOutFn() {
			...
			uni.setStorageSync('user_id', ''); // 清空分享上级的加密字符串		
			setTimeout(() => {
			that.$socket.loginout();//
			}, 2000);	
			uni.setStorageSync('chatlist',[])
			that.$store.commit('setToken', '');
			that.$store.commit('setmoney',0);
			that.$store.commit('setsore',0);
			that.$store.commit('setUserInfoAndPage', {
				userInfoAvatar: '', // 头像地址
				userInfoNickName: '', // 网名
				userInfoLv: 0, // 等级
